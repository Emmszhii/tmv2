...
class ReferralSource
{
    ...
    public $referral_source_start;
    public $referral_source_total;
    public $referral_source_search;
    ...
    public function readLimit()
    {
        try {
            ...
            $query = $this->connection->prepare($sql);
            $query->execute([
                "start" => $this->referral_source_start - 1,
                "total" => $this->referral_source_total,
            ]);
        } catch (PDOException $ex) {
            $query = false;
        }
    ...
    public function search()
    {
        try {
            ...
            $sql .= "* ";
            $sql .= "from {$this->tblReferralSource} ";
            $sql .= "where referral_source_name like :search ";  
            $sql .= "order by referral_source_is_active desc, ";
            $sql .= "referral_source_name asc ";
            $query = $this->connection->prepare($sql);
            $query->execute([
                "search" => "%{$this->referral_source_search}%",  
            ]);
        } catch (PDOException $ex) {
            $query = false;
        }
