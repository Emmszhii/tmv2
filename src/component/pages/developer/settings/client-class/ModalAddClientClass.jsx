...
const ModalAddClientClass = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `/v2/controllers/developer/settings/client-class/client-class.php?clientClassId=${itemEdit.client_class_aid}` //update
          : "/v2/controllers/developer/settings/client-class/client-class.php", //add
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ["settings-client-class"] });
      if (data.success) {
      ...
      <div className="bg-dark/50 fixed top-0 right-0 bottom-0 left-0 flex items-center justify-center z-50">
        ...
        >
          <div className="modal__header relative">
            <h3> {itemEdit ? "Update" : "Add"} Client Class </h3>
