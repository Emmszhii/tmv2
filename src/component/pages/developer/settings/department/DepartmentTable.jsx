...
import { getDepartmentCountRecord } from "./functions-general";
...
const DepartmentTable = ({ setItemEdit }) => {
  ...
  const handleArchive = (item) => {
    ...
    setId(item.settings_department_aid);
    setData(item);
    setDel(null);
  };
  ...
  return (
    <>
        ...
        <table>
          ...
          <tbody>
            ...
            {department?.data.map((item, key) => {
              active += item.settings_department_is_active === 1;
              inactive += item.settings_department_is_active === 0;
              return (
                <tr key={key}>
                  <td>{counter++}</td>
                  <td>
                    {item.settings_department_is_active === 1 ? (
                      <Pills />
                    ) : (
                      <Pills label="inactive" tc="text-archive" />
                    )}
                  </td>
                  <td>{item.settings_department_name}</td>
                  <td
      ...
      {store.isConfirm && (
        <ModalConfirm
          mysqlApiArchive={`/v2/controllers/developer/settings/department/active.php?departmentId=${id}`}
          msg={`Are you sure you want to archive this department?`}
          item={dataItem.settings_department_name}
      ...
      {store.isRestore && (
        <ModalDeleteAndRestore
          id={id}
          isDel={isDel}
          mysqlApiDelete={`/v2/controllers/developer/settings/department/department.php?departmentId=${id}`}
          mysqlApiRestore={`/v2/controllers/developer/settings/department/active.php?departmentId=${id}`}
          msg={
            isDel
