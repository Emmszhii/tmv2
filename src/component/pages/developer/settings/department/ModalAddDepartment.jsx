...
const ModalAddDepartment = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `/v2/controllers/developer/settings/department/department.php?departmentId=${itemEdit.settings_department_aid}` //update
          : "/v2/controllers/developer/settings/department/department.php", //add
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    settings_department_name: itemEdit ? itemEdit.settings_department_name : "",
    settings_department_name_old: itemEdit
      ? itemEdit.settings_department_name
      : "",
  };

  const yupSchema = Yup.object({
    settings_department_name: Yup.string().required("Required"),
  });
