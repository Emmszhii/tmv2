...
const ModalAddRates = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `/v2/controllers/developer/settings/rates/rates.php?ratesId=${itemEdit.settings_rates_aid}` //update
          : "/v2/controllers/developer/settings/rates/rates.php", //add
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ["settings-rates"] });
      if (data.success) {
  ...
  const initVal = {
    settings_rates_description: itemEdit
      ? itemEdit.settings_rates_description
      : "",

    settings_rates_description_old: itemEdit
      ? itemEdit.settings_rates_description
      : "",
  };
      ...
      <div className="bg-dark/50 fixed top-0 right-0 bottom-0 left-0 flex items-center justify-center z-50">
        ...
        >
          <div className="modal__header relative">
            <h3> {itemEdit ? "Update" : "Add"} Rates </h3>
